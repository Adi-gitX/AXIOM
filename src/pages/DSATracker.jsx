...
const DSATracker = () => {
    ...
    const solvedProblems = TOPICS.reduce((acc, topic) =>
        acc + topic.problems.filter(p => p.status === 'solved').length, 0
    );
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 glass-panel p-6 flex flex-col justify-between relative overflow-hidden">
                    ...
                    <div className="mt-6 flex items-end gap-2 text-primary font-display font-bold">
                        <span className="text-5xl">{solvedProblems}</span>
                ...
                <div className="glass-panel p-6 flex items-center justify-center relative">
                    ...
                    {/* Circular Progress Ring (Simple CSS) */}
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-textMuted text-sm">
                            {solvedCount} / {topic.total} Solved
                        </p>
                    </div>
                </div>
                <ChevronDown
                                ...
                                {topic.problems.map((prob) => (
                                    <div key={prob.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-surfaceHighlight/50 transition-colors group/item">
                                        <div className="flex items-center gap-3">
                                            <button className="text-textMuted hover:text-green-500 transition-colors">
                                                {prob.status === 'solved'
                                                    ? <CheckCircle2 size={20} className="text-green-500" />
                                                    : <Circle size={20} />
                                                }
                                            </button>
                                            <span className={`font-medium ${prob.status === 'solved' ? 'text-textMuted line-through' : 'text-text'}`}>
                                                {prob.title}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-4">
