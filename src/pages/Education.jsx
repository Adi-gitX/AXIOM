...
const Education = () => {
    ...
    return (
        <div className="min-h-screen p-8 lg:p-12">
                ...
                <motion.header initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-12">
                    {topic ? (
                        <div className="flex items-center gap-4">
                            <button onClick={() => setTopic(null)} className="w-10 h-10 rounded-full bg-white/10 border border-white/10 flex items-center justify-center text-white hover:bg-white/20 transition-colors">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {TOPICS.map((t, i) => (
                            <motion.button
                                ...
                                className="glass-card p-5 rounded-3xl text-left hover:bg-white/5 transition-all group border border-white/5 hover:border-white/20"
                            >
                                <span className="text-xs text-gray-500 font-mono group-hover:text-white transition-colors">{String(i + 1).padStart(2, '0')}</span>
                                <h3 className="font-medium text-white mt-2 group-hover:text-glow transition-all">{t.name}</h3>
                    ...
                    <div className="space-y-3">
                        {videos.map((v, i) => (
                            <motion.div
                                ...
                                onClick={() => setVideo(v)}
                                className="glass-card flex gap-5 p-4 rounded-3xl hover:bg-white/5 transition-all cursor-pointer group border border-white/5 hover:border-white/20"
                            >
                                <div className="w-48 aspect-video bg-black/50 rounded-xl overflow-hidden shrink-0 relative border border-white/10 group-hover:border-white/30 transition-all">
                                ...
                                <div className="flex-1 py-1">
                                    <h3 className="font-medium text-white group-hover:text-glow transition-all">{v.title}</h3>
                                    <p className="text-sm text-gray-400 mt-2">{v.channel}</p>
            ...
            <AnimatePresence>
                {video && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/90 backdrop-blur-xl flex items-center justify-center p-6" onClick={() => setVideo(null)}>
                        <button onClick={() => setVideo(null)} className="absolute top-6 right-6 w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-white/20 transition-colors border border-white/10">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-5xl glass-panel p-1 rounded-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="p-6">
                                <h2 className="text-white text-xl font-light text-glow">{video.title}</h2>
                                <p className="text-gray-400 mt-2">{video.channel} · {video.views}</p>
                            </div>
                        </motion.div>
                    </motion.div>
                )}
